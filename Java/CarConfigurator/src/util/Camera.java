...
public final class Camera {
	...
	private static int viewport_width = 720;
	private static int viewport_height = 500;
    ...
    public void updateProjection() {
    	float rechts = viewport_width/8;
		float links  = (-viewport_width/8);
	    float oben   = viewport_height/8;
	    float unten  = (-viewport_height/8);
        if(perspective) {
        	//(-0.01, 0.01, -0.01, 0.01, 0.01, 100.0)
            Util.frustum(-1.0f, 1.0f, -1.0f, 1.0f, 1.0f, 30.0f, projection);//-1e-2f, 1e-2f, -1e-2f, 1e-2f, 1e-2f, 1e+2f, projection);
        } else {
            Util.ortho(links, rechts, oben, unten, zNah, zFern, projection);
        }
    }
